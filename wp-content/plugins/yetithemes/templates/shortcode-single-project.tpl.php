...
if (strlen(trim($heading)) > 0) {
    ...
    echo '<div class="yeti-shortcode-header"><h3 class="' . esc_attr(implode(' ', $heading_class)) . '">' . esc_html($heading) . '</h3></div>';
}
    ...
    while ($products->have_posts()) : $products->the_post();
        ...
        if (class_exists('WooCommerce')) {
            ...
            if (isset($product_id) && $product_id !== '' && absint($product_id) > 0) {
                ...
                $rating_product = wc_get_rating_html( $product->get_average_rating() );
            }
        }

        if (function_exists('woothemes_get_testimonials')) {
            $testimonial_id = esc_attr(get_post_meta(get_the_ID(), '_testimonials_id', true));
            $yetithemes_testimonial = woothemes_get_testimonials(array('id' => $testimonial_id, 'limit' => 1, 'size' => '100'));
        }
        ...
        <div class="yeti-shortcode-content">
            ...
            if (count($yetithemes_testimonial) > 0) : ?>

                <p class="project-testimonial"><?php echo $yetithemes_testimonial[0]->post_content; ?></p>
