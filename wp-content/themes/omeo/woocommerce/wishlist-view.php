...
<div class="row">
    ...
    <div class="col-sm-17 col-md-19 woocommerce-MyAccount-content">
                ...
                <div class="wishlist-title <?php echo ($wishlist_meta['is_default'] != 1 && $is_user_owner) ? 'wishlist-title-with-form' : '' ?>">
                    ...
                    <?php if ($wishlist_meta['is_default'] != 1 && $is_user_owner): ?>
                        <a class="btn button show-title-form">
                            ...
                            <?php esc_html_e('Edit title', 'omeo') ?>
                        </a>
                    <?php endif; ?>
                </div>
                <?php if ($wishlist_meta['is_default'] != 1 && $is_user_owner): ?>
            ...
            <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr($pagination) ?>"
                ...
                <tr>
                    ...
                    <th class="product-name" colspan="2">
                        <span class="nobr"><?php echo apply_filters('yith_wcwl_wishlist_view_name_heading', esc_html__('Product Name', 'omeo')) ?></span>
                    </th>
                ...
                <span class="nobr">
                    <?php echo apply_filters('yith_wcwl_wishlist_view_price_heading', esc_html__('Unit Price', 'omeo')) ?>
                </span>
                        </th>
                ...
                <span class="nobr">
                    <?php echo apply_filters('yith_wcwl_wishlist_view_stock_heading', esc_html__('Stock Status', 'omeo')) ?>
                </span>
                        </th>
                ...
                if (count($wishlist_items) > 0) :
                        ...
                        if ($product !== false && $product->exists()) :
                            ...
                            <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>"
                                ...
                                <td class="product-name"
                                    data-title="<?php echo esc_html__('Product Name', 'omeo') ?>">
                                ...
                                <?php if ($show_price) : ?>
                                    <td class="product-price"
                                        data-title="<?php echo esc_html__('Price', 'omeo') ?>">
                                        <?php echo $product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_attr__( 'Free!', 'omeo' ) ); ?>
                                    </td>
                                <?php endif ?>

                                <?php if ($show_stock_status) : ?>
                                    <td class="product-stock-status"
                                        data-title="<?php echo esc_html__('Stock Status', 'omeo') ?>">
                                        <?php
                                        if ($stock_status == 'out-of-stock') {
                                            $stock_status = "Out";
                                            echo '<span class="wishlist-out-of-stock">' . esc_html__('Out of Stock', 'omeo') . '</span>';
                                        } else {
                                            $stock_status = "In";
                                            echo '<span class="wishlist-in-stock">' . esc_html__('In Stock', 'omeo') . '</span>';
                                        }
                                        ?>
                                    </td>
                                <?php endif ?>
                                        ...
                                        <div>
                                            <?php if ($show_last_column): ?>
                                                ...
                                                if ($show_dateadded && isset($item['dateadded'])):
                                                    echo '<span class="dateadded">' . sprintf(esc_html__('Added on : %s', 'omeo'), date_i18n(get_option('date_format'), strtotime($item['dateadded']))) . '</span>';
                                                endif;
                                                ?>
                                                ...
                                                <?php if ($available_multi_wishlist && is_user_logged_in() && count($users_wishlists) > 1 && $move_to_another_wishlist): ?>
                                                    <select class="change-wishlist selectBox">
                                                        <option value=""><?php esc_html_e('Move', 'omeo') ?></option>
                                                        <?php
                                                        foreach ($users_wishlists as $wl):
                                                            ...
                                                            <option value="<?php echo esc_attr($wl['wishlist_token']) ?>">
                                                                ...
                                                                if ($wl['wishlist_privacy'] == 1) {
                                                                    $wl_privacy = esc_html__('Shared', 'omeo');
                                                                } elseif ($wl['wishlist_privacy'] == 2) {
                                                                    $wl_privacy = esc_html__('Private', 'omeo');
                                                                } else {
                                                                    $wl_privacy = esc_html__('Public', 'omeo');
                                                                }
                                                ...
                                                <?php if ($is_user_owner && $repeat_remove_button): ?>
                                                    <a href="<?php echo esc_url(add_query_arg('remove_from_wishlist', $item['prod_id'])) ?>"
                                                       ...
                                                       title="<?php esc_attr_e('Remove this product', 'omeo') ?>"><?php esc_html_e('Remove', 'omeo') ?></a>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                            ...
                                            <?php if ($is_user_owner): ?>
                                                <div class="remove_wishlist">
                                                    <a href="<?php echo esc_url(add_query_arg('remove_from_wishlist', $item['prod_id'])) ?>"
                                                       ...
                                                       title="<?php esc_attr_e('Remove this product', 'omeo') ?>">&times;</a>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                    ...
                    <tr>
                        <td colspan="<?php echo esc_attr($column_count) ?>"
                            class="wishlist-empty"><?php esc_html_e('No products were added to the wishlist', 'omeo') ?></td>
                    </tr>
                    <?php
                endif;
                            ...
                            <?php if ($show_cb) : ?>
                                <div class="custom-add-to-cart-button-cotaniner">
                                    <a href="<?php echo esc_url(add_query_arg(array('wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token']))) ?>"
                                       class="button alt"
                                       id="custom_add_to_cart"><?php esc_html_e('Add the selected products to the cart', 'omeo') ?></a>
                                </div>
                            <?php endif; ?>
                                ...
                                <div class="ask-an-estimate-button-container">
                                    <a href="<?php echo ($additional_info) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>"
                                       class="btn button ask-an-estimate-button" <?php echo ($additional_info) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
                                        ...
                                        <?php esc_html_e('Ask for an estimate', 'omeo') ?>
                                    </a>
                                </div>
                            <?php endif; ?>
                        </td>
