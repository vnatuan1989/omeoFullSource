<?php
if (!class_exists('Yesshop_MetaBox_Template')) {
    class Yesshop_MetaBox_Template
    {
                ...
                <?php if (isset($options['pagram']) && is_array($options['pagram'])) : ?>
                    ...
                    <div class="nth_be_subtabs_content">
                        <?php foreach ($options['pagram'] as $k => $tab) :
                            ...
                            ?>
                            <div id="<?php echo esc_attr($tab['id']); ?>"
                                 class="<?php echo esc_attr(implode(' ', $class)); ?>" data-id="<?php esc_attr($k) ?>">
                                <?php if (isset($tab['pagram']) && is_array($tab['pagram'])) : ?>
                                    <ul class="list-options">
                                        <?php foreach ($tab['pagram'] as $element) : ?>
                                            <li><?php $this->createElements($element, $tab); ?></li>
            ...
            if (isset($options['name'])) {
                ...
                <div class="<?php echo esc_attr(implode(' ', $class)) ?>">
                    ...
                    <div class="yeti-form-field">
                        <input type="<?php echo esc_attr($options['type']) ?>"
                               placeholder="<?php echo esc_attr($placeholder); ?>"
                               name="<?php echo esc_attr($options['name']); ?>"
                               id="<?php echo esc_attr($options['name']); ?>"
                               value="<?php echo esc_attr($options['ntd']); ?>"/>
        ...
        private function render_size($options = array())
        {
            ...
            if (isset($options['name'])) {
                ...
                <div class="<?php echo esc_attr(implode(' ', $class)); ?>">
                    <?php if (isset($options['label'])) printf('<label>%s</label>', esc_html($options['label'])); ?>
                    <div class="yeti-form-field">
                        <input type="number" name="<?php echo esc_attr($options['name']); ?>[width]"
                               id="<?php echo esc_attr($options['name']); ?>_width"
                               value="<?php echo esc_attr($options['ntd']['width']); ?>" style='max-width: 65px;'/>&nbsp;&times;&nbsp;
                        <input type="number" name="<?php echo esc_attr($options['name']); ?>[height]"
                               id="<?php echo esc_attr($options['name']); ?>_height"
                               value="<?php echo esc_attr($options['ntd']['height']); ?>" style='max-width: 65px;'/>
                        <?php if (isset($options['desc']) && strlen($options['desc']) > 0) printf('<span class="desc">%s</span>', esc_html($options['desc'])); ?>
                    </div>
                </div>
                <?php
        ...
        private function render_textarea($options = array())
        {
            ...
            if (isset($options['name'])) {
                ...
                <div class="<?php echo esc_attr(implode(' ', $class)); ?>">
                    <?php if (isset($options['label'])) printf('<label>%s</label>', esc_html($options['label'])); ?>
                    <div class="yeti-form-field">
                        <textarea type="text" placeholder="<?php echo esc_attr($placeholder); ?>"
                                  name="<?php echo esc_attr($options['name']); ?>"
                                  id="<?php echo esc_attr($options['name']); ?>"><?php echo esc_html($options['ntd']); ?></textarea>
        ...
        private function render_media($options = array())
        {
            ...
            if (isset($options['name'])):
                ...
                ?>
                <div class="yeti-form-row<?php echo esc_attr($class); ?>">
                    <?php if (isset($options['label'])): ?>
                        <label><?php echo esc_html($options['label']); ?></label>
                    <?php endif; ?>
                    <div class="yeti-form-field nth_upload_media"
                         data-f_title="<?php esc_html_e('Choose an media', 'omeo'); ?>"
                         data-f_btext="<?php esc_html_e('Use Media', 'omeo'); ?>">
                        <input type="url" class="nth_media_output" name="<?php echo esc_attr($options['name']); ?>"
                               id="<?php echo esc_attr($options['name']); ?>"
                               value="<?php echo esc_url($options['ntd']); ?>"/>
                        <div class="controls">
                            <button type="submit" class="upload_media_button button button-primary"><i
                                        class="dashicons-before dashicons-admin-media"></i></button>
                            <button type="submit"
                                    class="remove_media_button button<?php if (!isset($options['ntd']) || strlen($options['ntd']) <= 0) echo " hidden"; ?>">
                                <i class="dashicons-before dashicons-trash"></i></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="<?php echo esc_attr(implode(' ', $class)); ?>"
                    ...
                    <div class="yeti-form-field">
                        <?php if (is_array($options['value']) && count($options['value']) > 0) : ?>
                            <select name="<?php echo esc_attr($options['name']); ?>"
                                    id="<?php echo esc_attr($options['name']); ?>">
        ...
        private function render_checkbox($options = array())
        {
            ...
            if (isset($options['name'])):
                ...
                <div class="<?php echo esc_attr(implode(' ', $class)); ?>">
                    <?php if (isset($options['label'])) printf('<label>%s</label>', esc_html($options['label'])); ?>
                    <div class="yeti-form-field">
                        <input type="checkbox" name="<?php echo esc_attr($options['name']); ?>"
                               id="<?php echo esc_attr($options['name']); ?>" <?php checked($options['ntd'], 1) ?>
                               value="1"/>
                        <?php if (isset($options['desc']) && strlen($options['desc']) > 0) printf('<span class="desc">%s</span>', esc_html($options['desc'])); ?>
                    </div><!-- .form-field -->
                </div>
                ...
                <div class="yeti-form-row">
                    ...
                    <?php if (is_array($options['value']) && count($options['value']) > 0) : ?>
                        <?php foreach ($options['value'] as $k => $v) :
                            ...
                            ?>
                            <input type="radio" name="<?php echo esc_attr($options['name']) ?>"
                                   id="<?php echo esc_attr($radio_id) ?>"
                                   value="<?php echo esc_attr($k); ?>" <?php checked($options['ntd'], $k); ?>>
                            <label class="option-label" for="<?php echo esc_attr($radio_id); ?>"><span
                                        class="nth-label <?php echo esc_attr($k) ?>"><?php echo esc_html($v) ?></span></label>
                            <br/>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
