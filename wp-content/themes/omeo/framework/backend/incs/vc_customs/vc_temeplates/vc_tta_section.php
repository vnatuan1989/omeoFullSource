...
if (absint(WPBakeryShortCode_VC_Tta_Section::$self_count) == 1 || empty($atts['use_ajax'])) {
    $output .= $this->getTemplateVariable('content');
} else {
    $output .= '<div class="hidden tab-ajax-content">' . urlencode(htmlspecialchars($content)) . '</div>';
}
