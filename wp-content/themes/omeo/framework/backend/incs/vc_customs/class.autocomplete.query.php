...
if (!class_exists("Yesshop_VC_Autocomplete_Query")) {

    abstract class Yesshop_VC_Autocomplete_Query
    {
        ...
        public function get_productCat_callback($query)
        {
            ...
            if (is_array($post_meta_infos) && !empty($post_meta_infos)) {
                foreach ($post_meta_infos as $value) {
                    ...
                    $data['label'] = 'ID' . ': ' .
                        $value['id'] .
                        ((strlen($value['name']) > 0) ? ' - ' . esc_html__('Name', 'omeo') . ': ' .
                            $value['name'] : '') .
                        ((strlen($value['slug']) > 0) ? ' - ' . esc_html__('Slug', 'omeo') . ': ' .
                            $value['slug'] : '');
                    $result[] = $data;
                }
            }
        ...
        public function get_productCat_render($query)
        {
            ...
            $term_slug_display = '';
            if (!empty($term_slug)) {
                $term_slug_display = ' - ' . esc_html__('Slug', 'omeo') . ': ' . $term_slug;
            }

            $term_title_display = '';
            if (!empty($term_title)) {
                $term_title_display = ' - ' . esc_html__('Name', 'omeo') . ': ' . $term_title;
            }

            $term_id_display = esc_html__('ID', 'omeo') . ': ' . $term_id;
