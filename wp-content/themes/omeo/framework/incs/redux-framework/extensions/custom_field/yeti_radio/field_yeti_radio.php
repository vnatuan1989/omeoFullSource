...
if (!class_exists('ReduxFramework_yeti_radio')) {
    class ReduxFramework_yeti_radio
    {
        ...
        function render()
        {
            ...
            if (!empty($this->field['options'])) {
                ...
                foreach ($this->field['options'] as $k => $v) {
                    ...
                    if (is_array($v)) {
                        if (!empty($v['title'])) echo '<strong>' . esc_html($v['title']) . '</strong>';
                        if (!empty($v['desc'])) echo '<p style="margin-left: 20px">' . esc_html($v['desc']) . '</p>';
                        if (!empty($v['img'])) echo '<p style="margin-left: 20px">' . Yesshop_Functions()->getImage(array('src' => esc_attr($v['img'])), false) . '</p>';
                    } else {
